...
func (q *Queries) CreateFeedFollow(ctx context.Context, arg CreateFeedFollowParams) (FeedFollow, error) {
	row := q.db.QueryRow(ctx, createFeedFollow, arg.ID, arg.FeedID, arg.UserID)
	var i FeedFollow
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
...
SELECT id, created_at, updated_at, feed_id, user_id FROM feed_follows WHERE user_id = $1 LIMIT $2
`
...
func (q *Queries) GetFeedFollows(ctx context.Context, arg GetFeedFollowsParams) ([]FeedFollow, error) {
	rows, err := q.db.Query(ctx, getFeedFollows, arg.UserID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
